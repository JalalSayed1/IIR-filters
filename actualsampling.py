...
def check_sampling_rate():
    ...
    if elapsed_time >= 1.0:  # Check every second
        ...
        print(f"Actual Sampling Rate: {actual_sampling_rate:.3f} Hz")
        sample_count = 0
...
def update_plot(frame):
    ...
    if current_sample is not None:
        ...
        #Doesn't update in plots:
        #ax_freq.set_title(f'Frequency Domain - Sampling Rate: {actual_sampling_rate:.2f} Hz')
...
# ani = animation.FuncAnimation(
#     fig, update_plot, init_func=init, blit=True, interval=1, cache_frame_data=False)
